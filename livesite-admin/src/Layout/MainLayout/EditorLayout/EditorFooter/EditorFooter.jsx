...
const EditorFooter = ({ setSelectSpecialFields }) => {
  ...
  const addCanvas = async () => {
    ...
    try {
      ...
      if (response?.status === "success") {
        const newCanvasId = `canvas-${response.pageID}`;
        const payload = {
          newCanvasId,
          userID,
          pageID: response.pageID,
          // layerId: response.layerId,
          calenderID: response.calenderID,
          backgroundID: response.backgroundID,
        };
        actions.addCanvas(payload);
  ...
  const deleteCanvas = async (canvasId) => {
    ...
    if (canvases.length === 1) {
      ...
      const newCanvasId = canvasId.split("-")[1];
      const response = await deleteApiCaller(`page/${newCanvasId}`, headers);
      if (response?.status === "success") {
                  ...
                  .map((canvas, index) => (
                    <Nav.Item key={index}>
                      <Dropdown show={openDropdownIndex === index} align="end">
                        <Dropdown.Toggle
                          className="rounded-0 editFooterNavLink"
                          as={Nav.Link}
