...
export default function PropertySidebar({
  ...
  useEffect(() => {
    if (selectedObject) {
      setHeight(parseInt(selectedObject.height * selectedObject.scaleY));
      setWidth(parseInt(selectedObject.width * selectedObject.scaleX));
      // setHeight(parseInt(selectedObject?.getScaledHeight()));
      // setWidth(parseInt(selectedObject?.getScaledWidth()));
      setXPosition(parseInt(selectedObject?.left) || 0);
      setYPosition(parseInt(selectedObject?.top) || 0);
    }
  }, [currentObjHeight, currentObjWidth, currentObjLeft, currentObjTop, selectedObject]);
    ...
    if (subCategoryList) {
      setSelectedSubCategory(
        subCategoryList.find(
          (res) => res.value === selectedObject?.SubCategoryID
        )
      );
    }
    if (categoryList) {
      setSelectedCategory(
        categoryList.find((res) => res.value === selectedObject?.CategoryID)
      );
    }
  ...
  const capitalizeFirstLetter = (text) => {
    ...
    return textLowercase.replace(/\b\w/g, (char) => char.toUpperCase());
  };
  ...
  const getFillValue = (field, value) => {
    if (field === "fillType") {
      switch (value) {
        case "Solid":
          return fillEnabled ? fillColor : "#ffffff";
        case "Gradient":
            ...
            selectedObject.type !== "image" && (
              <>
                    ...
                    <div className="col-lg-12 px-0 mt-3">
                      <CommonDropdown
                        ...
                        handleSelect={(e) => {
                          ...
                          actions.updateCurrentCategory(Number(e.value));
                        }}
                      />
                    </div>
                    <div className="col-lg-12 px-0 my-3">
                      <CommonDropdown
                        ...
                        handleSelect={(e) => {
                          ...
                          actions.updateCurrentSubCategory(Number(e.value));
                        }}
                      />
                    </div>
                  </>
                ...
                {selectedObject?.type !== "textbox" && (
                  <>
                    <hr className="hrColor py-0 my-0" />
                    <CommentSettings setSelectSpecialFields={setSelectSpecialFields}/>
                    <hr className="hrColor py-0 my-0" />
                    <TitleSettings />
                    <IconSettings />
                  </>
