...
import {
    ...
    defaultByDefaultShippingConfig
...
interface CommonBillingAddressProps {
...
const CommonBillingAddress: React.FC<CommonBillingAddressProps> = ({
    formik,
    handleCancel,
    ...
    const [shippingCheckboxChecked, setShippingCheckboxChecked] = useState(false);
    ...
    //                     Swal.fire(defaultByDefaultShippingConfig);
    //                   }
    //           setLoading(false)
    //           console.error(err)
    //         })
    ...
    const handleCheckboxClick = async () => {
        setShippingCheckboxChecked(!shippingCheckboxChecked);

        if (shippingCheckboxChecked) {
      ...
      const getAllShippingAddress = async () => {
        ...
        if (shippingAddressId) {
          try {
            const res = await axios.get(`${process.env.REACT_APP_SERVER_URL}/address/${shippingAddressId}`, {
              headers: { Authorization: 'Bearer ' + TOKEN?.AccessToken },
            });
      
            if (res.data.status === 'success') {
              const defaultShippingAddress = res.data.data;
              ...
              formik.setValues({
                ...
                setShippingCheckboxChecked(false)
              Swal.fire(defaultByDefaultShippingConfig);
            }
          } catch (err) {
            setShippingCheckboxChecked(false)
                    ...
                    <div className='d-flex pb-5 ms-auto justify-content-end'>
                        <div className='form-check formCustomCheck align-items-center'>
                            <input
                                ...
                                checked={shippingCheckboxChecked}
                            />
                            <label
...
export default CommonBillingAddress;
