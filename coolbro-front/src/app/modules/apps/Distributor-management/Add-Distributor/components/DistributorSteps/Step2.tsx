...
const Step2: FC<Step2Props> = ({editFlag,editDistributor}) => {
  ...
  useEffect(() => {
    if (editFlag) {
      let nameArray: string[] = editDistributor?.User?.Name.split(' ')
      // Extracting first and last names
      let firstName: string = nameArray[0]
      let lastName: string = nameArray.slice(1).join(' ')
      setSelectedOption(editDistributor?.UserDetails?.Gender.toLowerCase())
      ...
      // formik.setFieldValue('Phone_number_2', editDistributor?.UserDetails?.Contact_phone);

      formik.setFieldValue('Gender', editDistributor?.UserDetails?.Gender.toLowerCase())
      formik.setFieldValue('Email', editDistributor?.UserDetails?.Contact_email)

      formik.setFieldValue('Contact_email', editDistributor?.UserDetails?.Contact_email)
      formik.setFieldValue(
        'Date_of_birth',
        getFormattedDate(editDistributor?.UserDetails?.Date_of_birth)
      )
    }
