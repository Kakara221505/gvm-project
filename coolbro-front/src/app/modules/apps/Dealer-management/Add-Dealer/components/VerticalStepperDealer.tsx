...
import {Step1} from './DealerSteps/Step1'
import {Step2} from './DealerSteps/Step2'
import {Step3} from './DealerSteps/Step3'
import {Step4} from './DealerSteps/Step4'
import {Step5} from './DealerSteps/Step5'
...
import {defaultDealerAddSuccessful, defaultDealerUpdateSuccessful} from '../../../../../config/sweetAlertConfig'
...
const Vertical = () => {
  ...
  const [editDealer, setEditDealer] = useState<any>()
  ...
  useEffect(()=>{
    if(dealerId)
    {
      console.log("Dealer id exist", dealerId);
      axios
      .get(`${process.env.REACT_APP_SERVER_URL}/dealer/${dealerId}`, {
        headers: {Authorization: 'Bearer ' + TOKEN?.AccessToken},
      })
      .then((res) => {
        if (res.data) {
          let dealerData = res.data.data
          console.log('FROMAPI:', dealerData)
  ...
  const submitStep = (values: ICreateAccount, actions: FormikValues) => {
    ...
    if (stepper.current.currentStepIndex !== stepper.current.totalStepsNumber) {
      ...
      if(editFlag)
      {
         formData.set('id', editDealer?.UserDetails?.ID);
        ...
        formData.delete('MediaIDsToRemove')
          ...
          try {
            axios
              .post(
                `${process.env.REACT_APP_SERVER_URL}/dealer/add_update_dealerdetail`,
                formData,
                {
                  headers: {Authorization: 'Bearer ' + TOKEN?.AccessToken},
                }
              ...
              .then(async (res) => {
                ...
                if (res.status === 200 && res.data.message) {
                  ...
                  const confirmAdd = await Swal.fire(dealerId ? defaultDealerUpdateSuccessful :  defaultDealerAddSuccessful)
          ...
          {() => (
            <Form className='py-10 w-100  px-9' noValidate id='kt_create_account_form'>
              <div className='current' data-kt-stepper-element='content'>
                <Step1 editDealer={editDealer} editFlag={editFlag} />
              </div>

              <div data-kt-stepper-element='content'>
                <Step2 editDealer={editDealer} editFlag={editFlag}  />
              </div>

              <div data-kt-stepper-element='content'>
                <Step3  editDealer={editDealer} editFlag={editFlag}  />
              </div>

              <div data-kt-stepper-element='content'>
                <Step4 editDealer={editDealer} editFlag={editFlag}  />
              </div>

              <div data-kt-stepper-element='content'>
                <Step5 editDealer={editDealer} editFlag={editFlag}  />
              </div>
              ...
              <div className='d-flex flex-stack pt-10'>
                ...
                <div>
                  <button type='submit' disabled={loading} className='primaryBtn btn btn-sm me-3'     >
