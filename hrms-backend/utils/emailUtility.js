...
async function sendBirthAnniversayEmailTo(
  recipients,
  subject,
  content,
  ccRecipients = []
  ...
  try {
    ...
    // Ensure CC recipients are formatted as an array
    const ccEmails = Array.isArray(ccRecipients)
      ? ccRecipients
      : [ccRecipients];
    ...
    const mailOptions = {
      from: process.env.FROM_MAIL || "no-reply@company.com", // Replace with your email address
      to: toEmails.join(","), // Join array into a comma-separated string
      cc: ccEmails.join(","), // Include CC recipients if any
      subject, // Email subject
      html: content, // Email body content (HTML format)
    };
...
async function sendMailWithCC(recipients, subject, content, ccRecipients = []) {
  try {
    // Ensure recipients are formatted as an array
    const toEmails = Array.isArray(recipients) ? recipients : [recipients];
